...
test('preload exposes desktop version from BrowserWindow additionalArguments', (t) => {
  ...
  process.argv = [...originalArgv, `--hrs-desktop-version=${expectedVersion}`];
  ...
  assert.equal(exposeInMainWorldCalls[0][0], 'hrsDesktop');
  ...
  assert.equal(
    preloadModule.readDesktopVersion([`--hrs-desktop-version=${expectedVersion}`]),
    expectedVersion
  );
});
...
test('preload falls back to empty version when BrowserWindow does not pass one', (t) => {
  ...
  const preloadPath = require.resolve('../preload.js');
  delete require.cache[preloadPath];
  process.argv = originalArgv.filter((value) => !value.startsWith('--hrs-desktop-version='));
...
test('createDesktopBridge delegates update actions to ipcRenderer', async (t) => {
  ...
  assert.deepEqual(await desktopBridge.openReleasePage('https://github.com/ArcSeek-X/HermesX/releases/tag/v3.13.0'), {
    channel: preloadModule.DESKTOP_OPEN_RELEASE_PAGE_CHANNEL,
    payload: 'https://github.com/ArcSeek-X/HermesX/releases/tag/v3.13.0',
  });
