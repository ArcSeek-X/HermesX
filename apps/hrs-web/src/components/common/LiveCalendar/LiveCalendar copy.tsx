    ...
    return (
        <div
            className={cn(
                ...
                'flex w-full flex-col gap-0.5 rounded-sm pl-2 pr-1 pt-1 pb-1 mb-1 text-xs transition-colors',
                tone.bg,
                tone.text,
            )}
        >
            ...
            <div className="flex items-center gap-1">
                <span aria-hidden className={cn('shrink-0 text-lg font-bold leading-none', tone.text)}>·</span>
                {!event.isAllDay ? (
...
function GroupedEventContent({
    ...
    return (
        <div className="flex w-full flex-col rounded-sm bg-foreground-subtle px-0.5 py-1 transition-colors">
            {groups.map((group, gi) => {
                const tone = eventTone(group.importance);
                    ...
                    >
                        ...
                        <div className="flex flex-col">
                            {group.items.map((item, ii) => {
                                const itemTone = eventTone(item.importance);
                                return (
                                    <button
            ...
            <FullCalendar
                ...
                eventContent={(arg) => {
                    ...
                    if (groupEvents && groupEvents.length > 0) {
                        ...
                        const groups = eventGroupByTimeAndImportance(groupEvents);
                        return (
                            <GroupedEventContent groups={groups} onSelectEvent={onSelectEvent} />
                        );
                    }
                    return (
