...
interface PageHeaderProps {
  ...
  /** 外观模式，可选；'pill' 为胶囊卡片样式（有边框，含眉题与背景/边框/阴影），'default' 为标准精简模式（隐藏眉题 eyebrow，并移除背景/边框/阴影使其透明），用于更轻量的标题区。默认 'default'。 */
  mode?: 'default' | 'pill';
...
export const PageHeader: React.FC<PageHeaderProps> = ({
  ...
  return (
    <AnimCard className={cn('pl-2 pr-4 py-2 !min-h-0', mode === 'default' && '!bg-transparent !border-0 !shadow-none', className)}>
      {/* 语义标题区；w-full 抵消 AnimCard 根节点 flex 对子项宽度的收缩，保证宽屏下操作区贴右 */}
      <header className={cn('w-full ',mode === 'pill' && 'ml-2')}>
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            {mode === 'pill' && eyebrow ? <span className="text-xs leading-none tracking-[0.3em] uppercase">{eyebrow}</span> : null}
            <h1 className="mt-1 text-xl font-semibold tracking-tight text-foreground md:text-2xl">{title}</h1>
            {description ? <p className="mt-2 max-w-2xl truncate text-xs text-muted-text md:text-base">{description}</p> : null}
          </div>
