...
def test_dockerfile_uses_entrypoint_to_drop_privileges() -> None:
    ...
    assert "USER HRS" not in dockerfile
...
def test_dockerfile_bundles_default_alphasift_adapter() -> None:
    ...
    # 确认镜像内置默认的 AlphaSift 适配器；alphasift.dsa_adapter 是上游外部包模块真名，不可修改
...
def test_docker_entrypoint_repairs_ownership_and_user_permissions() -> None:
    ...
    assert "/home/hrs/.longbridge" in entrypoint
    assert 'HOME="/home/hrs"' in entrypoint
...
def test_docker_compose_injects_env_without_single_file_env_mount() -> None:
    ...
    assert "../longbridge_tokens:/home/hrs/.longbridge" in common["volumes"]
...
def test_documented_compose_exec_commands_run_as_hrs() -> None:
    safe_exec_prefix = "docker-compose -f ./docker/docker-compose.yml exec -u hrs"
...
def test_docker_entrypoint_skips_owner_chmod_when_chown_fails(tmp_path: Path) -> None:
    ...
    assert "still not writable by hrs" in result.stderr
