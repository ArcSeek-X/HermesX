...
docker-compose -f ./docker/docker-compose.yml exec -u hrs stock-analyzer bash

# Manually run analysis once
docker-compose -f ./docker/docker-compose.yml exec -u hrs stock-analyzer python main.py --no-notify
```
...
The Docker image startup entrypoint automatically creates and fixes ownership for the mounted `./data`, `./logs`, and `./reports` directories, then drops privileges to the non-root `hrs` user (UID 1000). Normal deployments do not require manual host-side `chown` / `chmod`.
...
git remote add origin https://github.com/your-username/HermesX.git
git branch -M main
