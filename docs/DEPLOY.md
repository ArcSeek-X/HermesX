...
Docker 镜像启动入口会自动创建并修复 `./data`、`./logs`、`./reports` 对应挂载目录的权限，然后降权为非 root 用户 (`hrs`, UID 1000) 运行应用。普通部署不需要手动 `chown` / `chmod`。
...
# 创建 GitHub 仓库并推送
# 在 GitHub 网页上创建新仓库后：
git remote add origin https://github.com/你的用户名/HermesX.git
git branch -M main
