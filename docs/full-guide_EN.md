...
1. Go to `Actions` tab
2. Select `HermesX-每日股票分析` workflow on the left
...
The image uses prebuilt frontend assets under `/app/static` at runtime, so the running `server` container does not require the `apps/hrs-web` source tree or runtime `npm`. If WebUI cannot be opened after Docker deployment, first verify that `/app/static/index.html` exists inside the container.

Official image registries:

- GHCR: `ghcr.io/ArcSeek-X/HermesX:<tag>`
- Docker Hub: `<DOCKERHUB_USERNAME>/HermesX:<tag>` (driven by the publisher's `DOCKERHUB_USERNAME` secret; the official release uses `ArcSeek-X/HermesX`)
...
For more questions, please [submit an Issue](https://github.com/ArcSeek-X/HermesX/issues)
