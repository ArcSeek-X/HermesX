...
- 解析与拉取阶段显式禁用环境代理（如 `HTTP_PROXY`、`HTTPS_PROXY`、`ALL_PROXY`），避免通过环境代理绕过校验边界；仅受信任的内置官方/聚合源（华尔街见闻快讯、NewsNow）在存在 fake-ip 代理（如 Clash 的 `198.18.0.0/15` 段）环境下允许复用系统代理，以便代理将其还原为真实公网地址；
- 实际连接阶段会再次校验目标主机 DNS 解析结果，避免校验后解析漂移到受限地址；受信任源放行 fake-ip 网段（非真实私网），其余内网/回环/保留判段逻辑不变；
- 重定向后的最终 URL 也会再次校验；
...
NewsNow 不是 RSS，而是一个聚合热点平台。HRS 直接按 HTTP API 读取它的 JSON 返回，不需要 MCP：
