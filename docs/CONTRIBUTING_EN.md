...
1. Search [Issues](https://github.com/ArcSeek-X/HermesX/issues) first to check if it has already been reported.
...
| `web-gate` | `npm run lint` + `npm run build` (triggered when `apps/hrs-web/` changes) | ✅ (when triggered) |
...
# Frontend gate (only if you changed apps/hrs-web/)
cd apps/hrs-web
npm ci
npm run lint
npm run build
