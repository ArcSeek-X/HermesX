...
| `EMAIL_SENDER_NAME` | 发件人显示名称（默认：HermesX股票分析助手） | 可选 |
...
1. 进入 `Actions` 标签
2. 左侧选择 `HermesX-每日股票分析` workflow
...
运行中的 `server` 容器默认直接复用 `/app/static` 里的预构建产物，不要求容器内保留 `apps/hrs-web` 源码目录或运行时安装 `npm`；若 WebUI 无法打开，请优先确认 `/app/static/index.html` 是否存在。
